...
var composeEnhancers = "node" === "web" && process.env.NODE_ENV === "development" && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ || redux.compose;
...
function createReduxStore() {
  ...
  var rootEnhancers = composeEnhancers.apply(undefined, [redux.applyMiddleware.apply(undefined, [apollo ? apollo.middleware() : emptyMiddleware,
  ...
  var store = redux.createStore(rootReducer, state, rootEnhancers);
...
var createHistory = require("history/createMemoryHistory")["default"];
...
function createReduxRouter(routes) {
  ...
  var history = path ? createHistory({
    initialEntries: [path]
  }) : createHistory();
...
function renderApplication(_ref) {
  ...
  var chunkNames = server.flushChunkNames();
...
function prepareResponse(request) {
  ...
  var browser = edgeUseragent.parse(request.headers["user-agent"]);
...
function isRelevantFrame(frame) {
  var wrappedFrame = sourceMapSupport.wrapCallSite(frame);
...
//# sourceMappingURL=node.es5.cjs.js.map
